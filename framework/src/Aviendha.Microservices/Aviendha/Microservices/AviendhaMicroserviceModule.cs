...
public class AviendhaMicroserviceModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        var redisConnStr = configuration[AviendhaMicroserviceKeys.AspireRedis];
        configuration[AviendhaMicroserviceKeys.RedisConfiguration] ??= redisConnStr ?? "localhost";
    }
    ...
    private static void ConfigureCors(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddCors(options =>
        {
            options.AddDefaultPolicy(builder =>
            {
                builder
                    .WithOrigins(
                        configuration[AviendhaMicroserviceKeys.CorsOrigins]!
    ...
    private static void ConfigureDistributedLocking(ServiceConfigurationContext context)
    {
        context.Services.AddSingleton<IDistributedLockProvider>(sp =>
        {
            var connection = sp.GetService<IConnectionMultiplexer>()
                ?? ConnectionMultiplexer.Connect(context.Services.GetConfiguration()[AviendhaMicroserviceKeys.RedisConfiguration]!);

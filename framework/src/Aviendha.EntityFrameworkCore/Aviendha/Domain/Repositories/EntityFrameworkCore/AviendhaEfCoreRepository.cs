...
using Aviendha.Application.Querying;
...
public class AviendhaEfCoreRepository<TDbContext, TEntity, TKey>(IDbContextProvider<TDbContext> dbContextProvider)
    : EfCoreRepository<TDbContext, TEntity, TKey>(dbContextProvider),
    IAviendhaEfCoreRepository<TEntity, TKey>
    ...
    where TEntity : class, IEntity<TKey>
{
    public virtual async Task<IQueryable<TEntity>> GetQueryableAsync(IQuerySpecification<TEntity> spec, CancellationToken cancellationToken = default)
        => (await GetQueryableAsync(/*cancellationToken*/)).GetQuery(spec);
}
